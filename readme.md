...
项目在 `evaluation/` 目录中提供标准问题集和检索评估程序。

评估内容：

- `Recall@K`：前 K 个结果中是否至少包含一个正确来源和页码
- `MRR`：第一个正确结果排名的倒数均值
- 来源文件和 PDF 页码匹配
- 不同 `k` 和距离阈值的参数对比
...
```powershell
python -m evaluation.evaluate_retrieval "vectorstores\uploads\知识库名称"
```

指定检索数量：

```powershell
python -m evaluation.evaluate_retrieval "vectorstores\uploads\知识库名称" --k 3
```

指定距离阈值：

```powershell
python -m evaluation.evaluate_retrieval "vectorstores\uploads\知识库名称" --k 6 --max-distance 0.8
```
...
当前小规模评估中，`k=6`、`max_distance=1.2` 的召回表现最好。由于标准问题数量仍然较少，该结果只作为当前知识库的基准，不代表对其他知识库同样最优。
...
标准问题保存在 `evaluation/questions.json`，后续应逐步扩充到 20–50 道，并覆盖事实题、解释题、跨文档题和无法回答的问题。